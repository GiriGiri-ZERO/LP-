      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="space-y-4">
          {content.items.map((item, i) => {
            const qEp = editableProps(`question[${i}]`);
            const aEp = editableProps(`answer[${i}]`);
            return (
              <div key={i} className="border-b border-gray-200 pb-5">
                <h3
                  ...
                  data-el-type="text"
                  className={`text-lg font-bold text-red-500 mb-2 ${qEp.className}`}
                  onBlur={isEditingEl(`question[${i}]`) ? (e) => {
                    const raw = e.currentTarget.textContent ?? "";
                    const newItems = content.items.map((it, j) =>
                      j === i ? { ...it, question: raw.replace(/^Q\.\s*/, "") } : it
                    );
                ...
                <p
                  ...
                  data-el-id={`answer[${i}]`}
                  data-el-type="text"
                  className={`text-gray-600 leading-relaxed ${aEp.className}`}
                  onBlur={isEditingEl(`answer[${i}]`) ? (e) => {
                    const raw = e.currentTarget.textContent ?? "";
                    const newItems = content.items.map((it, j) =>
                      j === i ? { ...it, answer: raw.replace(/^A\.\s*/, "") } : it
                    );
                    updateBlock(blockId, { items: newItems });
                  } : undefined}
                >
